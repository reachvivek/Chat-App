...
let membersAdded = [];
let groups;
const groupsUrl = "https://chat-app-nzjg.onrender.com/groups";
...
function getAddedMembers() {
  ...
  checkInput();
  groupNameInp.addEventListener("keydown", (e) =>
    e.code == "Enter" ? createGroup() : null
  );
  membersAdded = [];
...
function goToAddMembersPopup() {
  ...
  backBtn.addEventListener("click", hideAddMembersPopup);
  groupNameInp.removeEventListener("keydown", (e) =>
    e.code == "Enter" ? createGroup() : null
  );
}
...
function showChats(e) {
  ...
  let groupName;
  groups.map((group) =>
    group.id == groupId ? (groupName = group.name) : null
  );
