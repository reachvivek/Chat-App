...
function showGroupInfo(){
    axios({
        ...
        document.querySelector('.group-name').innerHTML=response.data.group.name
        document.querySelector('.group-info').innerHTML=`Group - ${members.length} participants`
...
function showAddParticipants(){
    ...
    document.getElementById('back-to-manage').addEventListener('click', hideAddParticipants)
    axios({
        method:'get',
        url:userUrl,
        headers:{'Authorization':state.token}
...
function hideAddParticipants(){
    document.querySelector('.add-participants').style.display='none'
    document.querySelector('.manage-group').style.display='flex'
    document.getElementById('back-to-manage').removeEventListener('click', hideAddParticipants)
}
